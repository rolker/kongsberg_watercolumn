...
def read_EMdgmHeader(data):
    ...
    header_size = struct.Struct(format_to_unpack).size
    fields = struct.unpack(format_to_unpack, data[:header_size])
    ...
    dg['dgdatetime'] = datetime.datetime.fromtimestamp(dg['dgtime'], datetime.timezone.utc)
    #dg['dgdatetime'] = datetime.datetime.utcfromtimestamp(dg['dgtime'])

    return dg, header_size

def read_EMdgmMpartition(data):
    """
    ...
    dg = {}
    format_to_unpack = "2H"
    datagram_size = struct.Struct(format_to_unpack).size
    fields = struct.unpack(format_to_unpack, data[:datagram_size])
    ...
    return dg, datagram_size

def read_EMdgmMbody(data):
    """
    ...
    return dg, dg['numBytesCmnPart']

def read_EMdgmMWCtxInfo(data):
    """
    ...
    return dg, dg['numBytesTxInfo']

def read_EMdgmMWCtxSectorData(data):
    """
...
def read_EMdgmMWCrxInfo(data):
    ...
    return dg, dg['numBytesRxInfo']

def read_EMdgmMWCrxBeamData(data):
    """
    ...
    extra_size = struct.Struct(format_to_unpack).size
    fields = struct.unpack(format_to_unpack, data[datagram_size:datagram_size+extra_size])

    dg['sampleAmplitude05dB_p'] = fields

    return dg, datagram_size+extra_size

def read_EMdgmMWCrxBeamPhase1(data, numSampleData):
    """
...
def read_EMdgmMWCrxBeamPhase2(data, numSampleData):
    """
    Read #MWC - Beam sample phase info, specific for each beam and water column sample.
...
def read_EMdgmMWC(data, full_payload = False):
    """
    ...
    dg = {}
    dg['header'], i = read_EMdgmHeader(data)
    dg['partition'], count = read_EMdgmMpartition(data[i:])
    i += count
    dg['cmnPart'], count = read_EMdgmMbody(data[i:])
    i += count
    if dg['partition']['numOfDgms'] > 1 and not full_payload:
        if dg['partition']['dgmNum'] == 1:
            i = 0 # to start with the whole packet
        dg['multibeam_payload'] = data[i:-4]
        dg['partitionNumBytes'], = struct.unpack('I', data[-4:])
        return dg
    dg['txInfo'], count = read_EMdgmMWCtxInfo(data[i:])
    i += count

    # Read TX sector info for each sector
    txSectorData = []
    for sector in range(dg['txInfo']['numTxSectors']):
        sector_data, count = read_EMdgmMWCtxSectorData(data[i:])
        txSectorData.append(sector_data)
        i += count
    #dg['sectorData'] = self.listofdicts2dictoflists(txSectorData)
    dg['sectorData'] = txSectorData
    dg['rxInfo'], count = read_EMdgmMWCrxInfo(data[i:])
    i += count
    ...
    for idx in range(dg['rxInfo']['numBeams']):
        beam_data, count = read_EMdgmMWCrxBeamData(data[i:])
        rxBeamData.append(beam_data)
        i += count
        ...
        if dg['rxInfo']['phaseFlag'] == 0:
            ...
            # TODO: Test with water column data, phaseFlag = 1 to complete/test this function.
            phase_info, count = read_EMdgmMWCrxBeamPhase1(data[i:], rxBeamData[idx]['numSampleData'])
            i += count
            rxPhaseInfo.append(phase_info)

        elif dg['rxInfo']['phaseFlag'] == 2:
            # TODO: Test with water column data, phaseFlag = 2 to complete/test this function.
            phase_info, count = read_EMdgmMWCrxBeamPhase2(data[i:], rxBeamData[idx]['numSampleData'])
            i += count
            rxPhaseInfo.append(phase_info)
